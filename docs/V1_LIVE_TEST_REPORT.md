...
- The strict concurrency build completes without concurrency warnings. The three existing Keychain API deprecation warnings remain.
- The release build, shell syntax checks, and whitespace checks pass.
- The ad hoc local package smoke test verifies version 1.1.3, the bundle identifier, plist, strict code signature, Intel and Apple silicon support, and the real empty main window.
- The smoke test uses temporary empty storage and does not read saved accounts or Keychain items.
- Privacy-safe main and Add Account captures are stored in `docs/design-qa`.
- A Standard Codex Security scan found nine valid low- or medium-severity issues. Version 1.1.3 fixes all nine before release.
- Reusable private-server links now use macOS Keychain. Migration rewrites both JSON generations so the current file and backup contain references only.
- The project-signed universal archive passes its checksum and project-signature checks. Its app reports version 1.1.3 build 113 and passes strict code-signature validation.
- The release archive contains no detected personal email, local user home path, personal name, `.DS_Store`, AppleDouble file, Git data, token, private key, or saved account data.
...
- The public repository kept version 1.1.1 as the latest release during the bridge period. Version 1.1.2 is now marked as latest.
...
- The August 18 transition source suite passed 206 tests with zero failures. Six explicit integration tests remained opt-in because they use installed Roblox clients, a published update, a local signed-app transition, or macOS approval UI.
...
- Version 1.1.1 was published on August 14, 2026 and stayed marked as latest during the bridge period so older updater clients could not skip it. Version 1.1.2 is now marked as latest.
