...
public actor ParallelRobloxLauncher {
    ...
    private func launchUnmodified(
        ...
        let expectedCodeHash = try codeDirectoryHash(at: applicationURL)
        ...
        do {
            try await verifyRunningProcess(
                processIdentifier,
                executablePath: expectedExecutablePath,
                codeDirectoryHash: expectedCodeHash
            )
    ...
    private func verifyRunningProcess(
        _ processIdentifier: Int32,
        executablePath: String,
        codeDirectoryHash expectedHash: String
    ) async throws {
        for _ in 0..<20 {
            if self.executablePath(for: processIdentifier) == executablePath,
               liveCodeDirectoryHash(for: processIdentifier) == expectedHash.lowercased() {
                return
            }
            guard kill(processIdentifier, 0) == 0 else { break }
            try? await Task.sleep(nanoseconds: 50_000_000)
        }
        throw RobloxLaunchError.unmodifiedParallelUnavailable(
            "The started Roblox process did not match the verified app copy."
        )
    }

    private func liveCodeDirectoryHash(for processIdentifier: Int32) -> String? {
        var code: SecCode?
        let attributes = [kSecGuestAttributePid as String: processIdentifier]
        ...
        let hash = values[kSecCodeInfoUnique] as? Data else { return nil }
    ...
    func prepareIsolatedEnvironment(for accountID: UUID) throws -> [String: String] {
        ...
        for directory in requiredDirectories {
            try rejectSymbolicLinksInManagedPath(through: directory)
    ...
    private func rejectSymbolicLinksInManagedPath(through target: URL) throws {
        let root = instancesRoot.standardizedFileURL
