...
public struct MetadataArchiveService: Sendable {
    ...
    public func importData(
        ...
        var importedAccountIDs = Set<UUID>()
        var importedUserIDs = Set<Int64>()
        guard archive.accounts.allSatisfy({
            importedAccountIDs.insert($0.id).inserted
                && importedUserIDs.insert($0.userID).inserted
        }) else {
            throw ArchiveError.duplicateAccountIdentity
        }
        var accounts = existingAccounts
        var accountIndexByUserID: [Int64: Int] = [:]
        var accountIDs = Set<UUID>()
        for (index, account) in accounts.enumerated() {
            accountIndexByUserID[account.userID] = accountIndexByUserID[account.userID] ?? index
            accountIDs.insert(account.id)
        }
        var importedCount = 0
        var accountIDMap: [UUID: UUID] = [:]
        for imported in archive.accounts {
            if let index = accountIndexByUserID[imported.userID] {
                ...
                if accountIDs.contains(imported.id) {
                    newAccount.id = UUID()
                }
                accounts.append(newAccount)
                accountIndexByUserID[newAccount.userID] = accounts.endIndex - 1
                accountIDs.insert(newAccount.id)
    ...
    public enum ArchiveError: LocalizedError {
        ...
        public var errorDescription: String? {
            ...
            case .duplicateAccountIdentity:
                return "This backup contains duplicate account identities and cannot be imported safely."
            }
        }
    }
}
