...
struct ContentView: View {
    ...
    let performsAutomaticChecks: Bool
    ...
    init(
        ...
        performsAutomaticChecks: Bool = true
    ) {
        self.store = store
        self.updater = updater
        self.performsAutomaticChecks = performsAutomaticChecks
    }
    ...
    var body: some View {
        ...
        .task {
            if performsAutomaticChecks {
                updater.checkIfNeeded()
                await store.checkAccountsOnStartup()
            }
        }
        .onAppear { AppSmokeTestConfiguration.reportMainWindowReady() }
